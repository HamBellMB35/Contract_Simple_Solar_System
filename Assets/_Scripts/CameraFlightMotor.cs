...
public class CameraFlightMotor : MonoBehaviour
{
    [Header("Dependencies")]
    [SerializeField] private CameraInputRouter inputRouter;
    [SerializeField] private SpaceEncyclopediaUIDriver uiDriver;

    // Private variables used to compute motion frame deltas
    private float _pitch;
    private Vector3 _targetPosition;
    private Vector3 _smoothedFlightVelocity; // Blends our flight vector over time for smooth momentum
    ...
    {
        // Subscribing to structural hooks inside the parser assembly
        if (inputRouter != null)
        {
            inputRouter.OnFocusPerformed += ExecuteFocus;
            inputRouter.OnUnfocusPerformed += BreakFollowState;
        }
    }

    private void OnDisable()
    {
        // Unhook tracking loops to eliminate background resource leakage
        if (inputRouter != null)
        {
            inputRouter.OnFocusPerformed -= ExecuteFocus;
            inputRouter.OnUnfocusPerformed -= BreakFollowState;
    ...
    {
        // Clear old UI overlays instantly if moving away to a new focus sequence
        if (uiDriver != null) uiDriver.ForceCloseUI();
        ...
        if (currenTarget != null)
        {
            ...
            if (planetIdentity != null && planetIdentity.bodyData != null)
            {
                ...
                if (uiDriver != null)
                {
                    ...
                    uiDriver.SetPromptVisibility(true);
                }
            }
            ...
            // Clamp the value strictly to prevent edge-case overflow stalls right at the vertical poles
            _pitch = Mathf.Clamp(rawPitch, -89.9f, 89.9f);
            // =====================================================================

        }
    }

    private void BreakFollowState()
    {
        _isFollowing = false;
        hoveredPlanetData = null;
        if (uiDriver != null) uiDriver.ForceCloseUI();
    ...
    private void HandleFollowing()
    {
        if (_isFollowing && currenTarget != null)
        {
            ...
            if (uiDriver != null && uiDriver.IsDataPanelOpen)
            {
    ...
    private void HandleKeyboardFlight()
    {
        ...
        Vector3 forwardFlight = transform.forward * inputRouter.MovementInput.y;
        Vector3 sidewaysFlight = transform.right * inputRouter.MovementInput.x;
        Vector3 verticalFlight = transform.up * inputRouter.VerticalInput;
    ...
    // Public void HandleOrbiting() will be called in the LateUpdate method, and it will use the MouseDelta and IsOrbiting properties to determine how to rotate the camera
    public void HandleOrbiting()
    {
        if (inputRouter.IsOrbiting)
        {
            // We update our 'internal record' of the pitch.
            // We subtract because in most games, moving mouse UP should decrease the X-angle.
            _pitch -= inputRouter.MouseDelta.y * _rotationSensitivity_Y;
            ...
            transform.rotation = Quaternion.Euler(_pitch, transform.eulerAngles.y + (inputRouter.MouseDelta.x * _rotationSensitivity_X), 0);
    ...
    public void HandlePanning()
    {
        if (inputRouter.IsPanning)
        {
            ...
            Vector3 panMove = new Vector3(-inputRouter.MouseDelta.x, -inputRouter.MouseDelta.y, 0) * _panningSensitivity;
    ...
    public void HandleZooming()
    {
        ...
        if (Mathf.Abs(inputRouter.ScrollInput) > 0.01f)
        {
            // Calculate the direction based on where the camera is facing, and the scroll input
            Vector3 zoomDir = transform.forward * inputRouter.ScrollInput * _zoomSensitivity;
            ...
            if (_isFollowing)
            {
                ...
                _followDistanceAnchor -= inputRouter.ScrollInput * _zoomSensitivity;
